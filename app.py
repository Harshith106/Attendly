...
# Limit concurrent Playwright browser launches (tune this based on your server's capacity)
import threading
# Set the maximum number of concurrent scrapes. Tune this based on your server's CPU/RAM.
# For Render free plan, 1-2 is safe. For paid plans, increase as needed.
...
print(f"[INIT] MAX_CONCURRENT_SCRAPES set to {MAX_CONCURRENT_SCRAPES}")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            try:
                ...
                logging.error(f"Error waiting for attendance data: {str(e)}")
            # Now fetch student name after data is loaded
            ...
            try:
                ...
                # Now fetch attendance fieldsets
                try:
                ...
                # Calculate correct summary using total attended/conducted (not average of percentages)
                total_attended = sum(classes_attended)
                total_conducted = sum(classes_conducted)
                overall_percentage = (total_attended / total_conducted * 100) if total_conducted else 0
                ...
                total_attended_no_tt = sum(classes_attended_no_tt)
                total_conducted_no_tt = sum(classes_conducted_no_tt)
                overall_percentage_no_tt = (total_attended_no_tt / total_conducted_no_tt * 100) if total_conducted_no_tt else 0
...
def scrape():
    ...
    if not acquired:
        logging.warning(f"[HEAVY LOAD] Server busy: too many concurrent scrape requests. User: {username}")
        print(f"[HEAVY LOAD] Rejecting request for user {username} due to max concurrency.")
        return jsonify({'error': 'Server is currently handling maximum load. Please wait a few seconds and try again.'}), 429
    import time
    ...
    try:
        ...
        print(f"[SCRAPE] Finished at: {end_time}")
        print(f"[SCRAPE] Time taken to calculate and fetch result: {duration:.2f} seconds.")
        if result:
